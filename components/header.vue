<template>
    <header v-if="header" :class="isMobileMenuOpen ? 'fixed' : ''">
        ...
        <img
            :src="isMobileMenuOpen ? '/close.svg' : '/bars.svg'"
        ...
        <nav :class="isMobileMenuOpen ? 'open' : 'closed'">
            <ul class="first-level">
                <li v-for="(item, index) of firstLevelNavItems" :key="index">
                    ...
                    <span v-else class="no-link mobile-only">{{
                        item.name
                    }}</span>
                    ...
                    <span v-else class="no-link desktop-only">{{
                        item.name
                    }}</span>
                </li>
...
export default {
    ...
    methods: {
        ...
        name(item) {
            return (
                (item.name[0] && item.name[0].text) ||
                this.$store.getters.page(item.page.uid).title[0].text
            );
        },
        href(item) {
            return (
